...
class DAO_Usuario():
    ...
    def concultarTodos(self):
        sql = """
            SELECT Id, Nome, Email, Senha FROM TBL_Usuarios
            """
        try:
            conn = ConnectionFactory.get_Connection()
            cursor = conn.cursor()
    ...
    def consultarNomesId(self):
        sql = """
              SELECT Nome, Id FROM TBL_Usuarios
              """
        try:
            conn = ConnectionFactory.get_Connection()
            cursor = conn.cursor()
    ...
    def consultarInformacoesId(self, Id):
        try:
            sql = """        
                  SELECT Nome, Email, Senha FROM TBL_Usuarios WHERE Id = ?
                  """
            conn = ConnectionFactory.get_Connection()
            cursor = conn.cursor()
            cursor.execute(sql, (Id,))
            informacoesUsuario = cursor.fetchall()
            cursor.close()
            conn.close()
